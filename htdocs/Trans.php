					...
					if (isset($Phone['UserID'])) 
					{
						if ($User['UserID'] != $Phone['UserID'])
						{
							if (!preg_match("/\D/", $_POST['Gold']))
							{
								...
								if ( $_POST['Gold'] >= 1 )
								{
									if ($User['Gold'] >= $_POST['Gold'])
									{
										$printTransForm = false;
										
										if (isset($_POST['Confirm']))
										{
											$FromGold = $User['Gold'] - $_POST['Gold'];
											$ToGold = $Phone['Gold'] + $_POST['Gold'];
											
											$sql = "UPDATE users SET Gold=? WHERE UserID=?";
											$upFromGold = $db_connect->prepare($sql);
											$upFromGold->execute([$FromGold,$User['UserID']]);
											
											$sql = "UPDATE users SET Gold=? WHERE UserID=?";
											$upToGold = $db_connect->prepare($sql);
											$upToGold->execute([$ToGold,$Phone['UserID']]);
											
											$insertTransLog = $db_connect->prepare("INSERT INTO gold_log(GiverID,TakerID,GiverGold,TakerGold,TheGive) values(?,?,?,?,?)");
											$insertTransLog->execute(array($User['UserID'],$Phone['UserID'],$User['Gold'],$Phone['Gold'],$_POST['Gold']));
										...
										echo '<h1 style="direction: rtl;color: #ff0;">الى '.$Phone['Surname'].' </h1>';
	...
	if ( $printTransForm ) { 
		...
		if (!$User['Auth']) {
			echo'<form action="Auth.php">
				<input class="inputA" type="submit" value="تفعيل صلاحيات التحويل" style="background: #ff9800" />
			</form>';
		}
...
function printTransForm($User,$db_connect)
{
	...
	echo '<input class="inputA" type="submit" value="تحويل الرصيد" style="background: #4caf50" />';
	echo '</select></form>';
}