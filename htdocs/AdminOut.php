						...
						if (!preg_match("/\D/", $_POST['Gold']))
						{
							...
							if ( $_POST['Gold'] >= 1 )
							{
								if ($_POST['Gold'] <= $Phone['Gold'])
								{
									...
									echo '<h1 style="direction: rtl;color: #ff0;">سيتم سحب مبلغ '.$_POST['Gold'].' مللي</h1>';
									echo '<h1 style="direction: rtl;color: #ff0;">من حساب '.$Phone['Surname'].' </h1>';
									...
									echo '<input id="trans" class="inputA" type="submit" value="سحب الرصيد" style="background: #4caf50" />';
									echo '</form>';
								...
								} else { echo '<p style="color: #f00;">لا يوجد رصيد كافي</p>'; }
							} else { echo '<p style="color: #f00;">مينفعش تحول أقل من 1 ملل</p>'; }
...
function printTransForm($User,$db_connect)
{
	...
	echo '<input class="inputA" type="submit" value="سحب الرصيد" style="background: #4caf50" />';
	echo '</select></form>';
	
	echo'<form action="AdminIN.php">
			<input class="inputA" type="submit" value="إيداع رصيد" />
		</form>';
}